...
;(function ($) {
    ...
    function hideBanner() {
        $(document).off('keydown.cookiePolicyModal');
        $('body').css('overflow', '');
        $('#cookie-consent-layer').fadeOut(120, function () {
            $(this).remove();
        });
    }

    function showSaveError() {
        if ($('#cookie-consent-error').length) {
            return;
        }
        $('<div id="cookie-consent-error" class="cookie-consent-error">Не удалось сохранить выбор. Проверьте соединение и попробуйте еще раз.</div>').appendTo('#cookie-consent-banner .cookie-consent-banner__inner');
    }

    function openPolicyModal() {
        var sourceHtml = $('#cookie-policy-static-source').html() || '';
        var modal = $('#cookie-policy-modal');
        lastFocusedElement = document.activeElement;
        $('#cookie-policy-modal-body').html(sourceHtml);
        modal.fadeIn(120).attr('aria-hidden', 'false');
        modal.find('.js-cookie-policy-close').focus();
    }

    function closePolicyModal() {
        $('#cookie-policy-modal').fadeOut(120).attr('aria-hidden', 'true');
    ...
    function saveConsent(action) {
        var endpoint = action === 'accept' ? '/cookie-consent/accept/' : '/cookie-consent/reject/';
        $('#cookie-consent-error').remove();
        $.ajax({
            url: endpoint,
            type: 'POST',
            dataType: 'json',
    ...
    $(function () {
        ...
        banner.on('click', '.cookie-consent-action', function () {
            var action = $(this).data('consent');
            if (action !== 'accept' && action !== 'reject') {
                return;
            }
            saveConsent(action);
        });
        banner.on('click', '.js-cookie-policy-open', function (event) {
            event.preventDefault();
            openPolicyModal();
        });
        $('#cookie-consent-layer').on('click', '.js-cookie-policy-close', function () {
            closePolicyModal();
        });
        $(document).on('keydown.cookiePolicyModal', function (event) {
            if (event.keyCode === 27) {
                closePolicyModal();
            }
        });
...
var $ = jQuery
var vue_simple_store = {
    debug: true,
    state: {
        favs: [],
        cart_items: [],
        cart_items_by_id: {},
        cat_data: [{'name': 'Product name 1', 'id': "0001"}, {
            'name': 'Product name 2',
            'id': "0002"
        }, {'name': 'Product name 3', 'id': "0003"}, {
            'name': 'Product name 4',
            'id': "0004"
        }, {'name': 'Product name 5', 'id': "0005"},],
        URLS: window.URLS
    },
    setVSSItem(key, val) {
        if (this.debug) console.log('setVSSItem triggered with', val)
        this.state[key] = val
    },
    setFavsAction(newValue) {
        if (this.debug) console.log('setFavsAction triggered with', newValue)
        this.state.favs = newValue
    },
    setCartItemsAction(newValue) {
        var self = this
        if (this.debug) console.log('setCartItemsAction triggered with', newValue)
        this.state.cart_items = newValue
        self.state.cart_items_by_id = {}
        _.each(self.state.cart_items, function (i) {
            self.state.cart_items_by_id[i.id] = i.amount
        })
    }
}
Vue.component('fav-button', {
    template: '#vue_fav_button',
    delimiters: ['${', '}'],
    props: {product_id: {type: Number}, big: {type: Boolean, default: false}},
    data: function () {
        return {vss: vue_simple_store.state}
    },
    methods: {
        onClick: function ($e) {
            $e.preventDefault();
            var self = this, $this = $(self.$el),
                notification_timeout = parseInt($this.data('notification_timeout')) || 5000,
                source_target = this.source_target, icon_selector = $this.data('icon'), $to_block = '',
                icon_class = $this.data('icon_success_class'),
                icon_success_class = $this.data('icon_class') || icon_class,
                icon_wait_selector = $this.data('icon_wait'), remove_path = $this.data('remove_path'), post_data = {};
            $.each($this.data(), function (k, val) {
                if (k.indexOf('postdata') !== -1) {
                    post_data["" + k.replace('postdata_', '').replace('postdata', '').toLowerCase()] = val;
                }
            });
            post_data = {action: this.action, product_id: this.product_id, owner: 0}
            $this.find(icon_selector).toggleClass(icon_class).toggleClass(icon_wait_selector);
            $.ajaxSetup({
            ...
            $($to_block).each(function () {
                $(this).block({
                    css: {border: 'none', background: 'none'},
                    overlayCSS: {cursor: 'wait', padding: '0px'},
                    message: '<i class="fa fa-circle-o-notch fa-inverse fa-spin fa-2x"></i>'
                })
            });
            $.ajax({
                type: "POST", url: self.action_url, data: post_data, success: function (data) {
                    try {
                        data = $.parseJSON(data);
                    } catch (e) {
                    }
                    try {
                        $($to_block).unblock();
                        var tpl = get_template('add_to_cart_success');
                        var msg_tpl = '<%= message %>';
                        tpl = _.template(msg_tpl)({'message': data.message});
                        if (remove_path && data.parent_pk) {
                            remove_product_widget_from_list(data.parent_pk, remove_path)
                        }
                        data.life = notification_timeout
                        if (data.favs) {
                            try {
                                vue_simple_store.setFavsAction(data.favs)
                            } catch (e) {
                                console.debug(e)
                            }
                        }
                    ...
                    console.log(e);
                    $this.find(icon_selector).toggleClass(icon_wait_selector).toggleClass(icon_class);
                    $($to_block).unblock();
                    var tpl = get_template('common_error');
                    tpl = _.template(tpl)();
                    $.jGrowl({life: notification_timeout, theme: 'alert alert-error'});
                    make_qty_buttons();
                }
            });
        }
    ...
    computed: {
        in_favorite: function () {
            return this.vss.favs.includes(this.product_id)
        },
        fav_classes: function () {
            var classes = ' add_remove_favorite  ', icon_class = 'fa fa-heart-o'
            if (this.in_favorite) {
                classes += ' in_favorite added'
                icon_class = 'fa fa-heart'
            }
            return {a_class: classes, icon_class: icon_class}
        },
        action_url: function () {
            return (this.in_favorite ? this.vss.URLS.fav_del : this.vss.URLS.fav_add) + '?ajax'
        },
        action: function () {
            return this.in_favorite ? 'delete' : 'add'
        },
        target_selector: () => '#' + (this.in_favorite ? 'delete' : 'add') + '_favorite_' + this.product_id,
        source_target: () => this.target_selector
    }
})
Vue.component('fav-icon', {
    template: '#vue_fav_icon', delimiters: ['${', '}'], data: function () {
        return {vss: vue_simple_store.state}
    }, computed: {
        favs_count: function () {
            return this.vss.favs.length
        }
    }
})
Vue.component('cart-icon', {
    template: '#vue_cart_icon', delimiters: ['${', '}'], data: function () {
        return {vss: vue_simple_store.state}
    }, computed: {
        cart_items_count: function () {
            var n = 0, items = this.vss.cart_items || [];
            for (var i = 0; i < items.length; i++) {
                n += Number(items[i].amount) || 0
            }
            return n
        }
    }
})
Vue.component('in-cart-badge', {
    template: '#vue_in_cart_badge',
    delimiters: ['${', '}'],
    props: ['var_id'],
    data: function () {
        return {vss: vue_simple_store.state}
    ...
    computed: {
        amount: function () {
            return this.vss.cart_items_by_id[this.var_id] || ''
        }
    }
})
Vue.component('vue_test_bots', {
    template: '#vue_test_bots', delimiters: ['${', '}'], data: function () {
        return {vss: vue_simple_store.state, local: "local state", apiData: []}
    }, computed: {
        favs_count: function () {
            return this.vss.favs.length
        }
    }, mounted() {
        console.log('vue_test_bots mounted')
        var self = this
        $.get('/vue_test_api_data/').success((data) => {
            self.apiData = JSON.parse(data)
        })
    }
})
var app = new Vue({
...
}(jQuery, window, document);/*! jQuery UI - v1.11.4 - 2016-03-01
* http://jqueryui.com
...
function collapse_xs_sm() {
    if (window.innerWidth < 767) {
        $('.collapse-xs').removeClass('show')
        $('.collapse-sm').removeClass('show')
    } else {
        $('.collapse-xs').addClass('show')
...
(function ($) {
    ...
    $('.meanmenu-reveal').on('click', function (e) {
        e.preventDefault();
        $('.mean-nav').toggle();
        $(this).toggleClass('meanmenu-reveal-fixed');
    })
    ...
    $('.sidebar-category').on('click', 'li.has-children > a', function (e) {
        if ($(this).parent().hasClass('has-children')) {
            if ($(this).siblings('ul:visible').length > 0) $(this).siblings('ul').slideUp(); else {
                $(this).parents('li').siblings('li').find('ul:visible').slideUp();
    ...
    $('.product-slider-4').slick({
        ...
        }, {breakpoint: 767, settings: {autoplay: true, slidesToShow: 1, arrows: false,}}]
    });
    $('.product-slider-4-full').slick({
        ...
        prevArrow: '<button type="button" class="slick-prev"><i class="icofont icofont-long-arrow-left"></i></button>',
        nextArrow: '<button type="button" class="slick-next"><i class="icofont icofont-long-arrow-right"></i></button>',
        responsive: [{breakpoint: 1499, settings: {slidesToShow: 3,}}, {
            breakpoint: 1199,
            settings: {slidesToShow: 2,}
        }, {breakpoint: 991, settings: {slidesToShow: 2,}}, {
            breakpoint: 767,
            settings: {autoplay: true, slidesToShow: 1, arrows: false,}
        }]
    });
    $('.product-slider-3').slick({
        ...
        prevArrow: '<button type="button" class="slick-prev"><i class="icofont icofont-long-arrow-left"></i></button>',
        nextArrow: '<button type="button" class="slick-next"><i class="icofont icofont-long-arrow-right"></i></button>',
        responsive: [{breakpoint: 1199, settings: {slidesToShow: 4,}}, {
            breakpoint: 991,
            settings: {slidesToShow: 3,}
        }, {breakpoint: 767, settings: {autoplay: true, slidesToShow: 2, arrows: false,}}, {
            breakpoint: 600,
            settings: {autoplay: true, slidesToShow: 1, arrows: false,}
        }]
    });
    $('.big-image-slider').slick({arrows: false, dots: true, slidesToShow: 1,});
    $('.team-slider-5').slick({
        arrows: false,
        dots: false,
        autoplay: false,
        ...
        responsive: [{breakpoint: 1199, settings: {slidesToShow: 4,}}, {
            breakpoint: 991,
            settings: {slidesToShow: 3,}
        }, {breakpoint: 767, settings: {autoplay: true, slidesToShow: 2,}}, {
            breakpoint: 479,
    ...
    $('.brand-slider').slick({
        ...
        }, {breakpoint: 767, settings: {slidesToShow: 3,}}, {breakpoint: 479, settings: {slidesToShow: 2,}}]
    });
    ...
    imagePopup.magnificPopup({type: 'image', mainClass: 'mfp-fade',});
    var galleryPopup = $('.gallery-popup');
    galleryPopup.magnificPopup({type: 'image', mainClass: 'mfp-fade', gallery: {enabled: true,},});
    var counter = $('.counter');
    counter.counterUp({delay: 20, time: 3000});
    $('#mc-form').ajaxChimp({
        language: 'en',
    ...
    $('#price-range').slider({
        range: true, min: 0, max: 2000, values: [25, 970], slide: function (event, ui) {
            $('#price-amount').val('$' + ui.values[0] + ' TO $' + ui.values[1]);
        }
    });
    $('#price-amount').val('$' + $('#price-range').slider('values', 0) + ' TO $' + $('#price-range').slider('values', 1));

    function make_qty_buttons() {
    ...
    $('.field-filled').on('click', function (e) {
        e.preventDefault()
        if ($(this).hasClass('names__link') || $(this).find('.names__link').length) {
            $('.edit__names').each(function () {
                $(this).addClass('edit-mode');
            })
        } else {
            $(this).addClass('edit-mode').siblings('.edit-wrap').addClass('edit-mode')
        }
    });
    $('.checkout-choice-link').on('click', function (e) {
        e.preventDefault()
        $(this).parent().parent().find('.checkout-choice-link').removeClass('checkout-choice-link__current')
        $(this).addClass('checkout-choice-link__current')
        $(this).parents('.edit-wrap').first().find('input').val($(this).html().trim()).change()
    })
    $('.checkout-field-save').on('click', function (e) {
        e.preventDefault()
        if ($(this).hasClass('names__link') || $(this).hasClass('edit__names')) {
            $('.field-filled.edit__names').removeClass('edit-mode')
            $('.edit-wrap.edit__names').removeClass('edit-mode')
        } else {
            $(this).parents('.edit-wrap').first().removeClass('edit-mode').siblings('.field-filled').removeClass('edit-mode')
        }
    })
    $('.radio-switch').on('click', function (e) {
        e.preventDefault()
        $('.radio-switch').toggleClass('radio-switch__active')
    })
    collapse_xs_sm()
})(jQuery);
;var $body = $('body');

function block($el, block_message = '', block_spinner = '', block_css = '', overlay_css = '', spinner_size = '1') {
    if (typeof $el === 'undefined') {
        $el = $body;
    }
    block_message = '<div style="text-align: center; margin-top: 30px; color: #fff; font-size: 24px">' + block_message + '</div>';
    if (block_spinner === '') {
        block_spinner = '<i class="fa fa-circle-o-notch fa-' + spinner_size + 'x fa-inverse fa-spin"></i>';
    }
    if (block_css === '') {
        block_css = {borderRadius: '6px', border: 'none', background: 'none', paddingTop: '10px'};
    }
    var full_message = block_spinner + block_message;
    $($el).block({css: block_css, overlayCSS: {cursor: 'wait'}, message: full_message});
}

function add_to_cart() {
    console.log('add_to_cart_handler');
}
...
;(function ($) {
    ...
    $.extend($.fn.jGrowl.prototype, {
        defaults: {
            ...
            var notification = $('<div class="jGrowl-notification"><div class="close">&times;</div><div class="header">' + o.header + '</div><div class="message">' + message + '</div></div>').data("jGrowl", o).addClass(o.theme).children('div.close').bind("click.jGrowl", function () {
            ...
            if (false) $(this.element).addClass('ie6');
        }, shutdown: function () {
...
;$.fn.setCaret = function (b, c) {
    var d;
    null == b && (b = 0);
    null == c && (c = b);
    null != window.getSelection && (d = window.getSelection()).removeAllRanges();
    this.each(function () {
        var a, e;
        try {
            if (null != (e = this.childNodes) && e.length) {
                if (null != document.createRange) return a = document.createRange(), a.setStart(this.childNodes[0], b), a.setEnd(this.childNodes[0], c), d.addRange(a)
            } else {
                if (null != this.setSelectionRange) return this.setSelectionRange(b, c);
                if (null != this.createTextRange) return a = this.createTextRange, a.collapse(!0), a.moveEnd("character", c), a.moveStart("character", b), a.select();
                this.selectionStart = b;
                return this.selectionEnd = c
            }
        } catch (f) {
        }
    });
...
$.fn.setCaretFromPoint = function (b, c) {
    var d, a;
    null == b && (b = 0);
    null == c && (c = 0);
    try {
        null != document.caretPositionFromPoint ? (d = document.caretPositionFromPoint(b, c), this.setCaret(d.offset)) : null != document.caretRangeFromPoint ? (a = document.caretRangeFromPoint(b, c), this.setCaret(a.startOffset)) : null != document.body.createTextRange && (a = document.body.createTextRange(), a.moveToPoint(b, c), a.select())
    } catch (e) {
    }
    return this
};
    ...
    var methods = {
        _init: function (el) {
            ...
            if (options.type == 'overlay' && !options.absolute && options.background) {
                ...
                $wrapper[0].style.WebkitOverflowScrolling = 'touch';
            }
            $el.css({opacity: 0, visibility: 'hidden', 'pointer-events': 'auto', display: 'inline-block'});
            if (options.setzindex && !options.autozindex) {
            ...
            if (options.type == 'overlay') {
                ...
                $wrapper.css({position: 'fixed', width: '100%', height: '100%', top: 0, left: 0, textAlign: 'center'});
            ...
            if (options.type == 'tooltip') {
                ...
                if (options.absolute) {
                    $wrapper.css({position: 'absolute', top: window.scrollY});
                }
                if (!options.background) {
                    $wrapper.css({'pointer-events': 'none'});
                    if (!options.absolute && !isInViewport(el)) {
                        $el.css('overflow', 'auto');
                        $el[0].style.WebkitOverflowScrolling = 'touch';
...
function pushEcommerceToDataLayer(ecommerce, context) {
    try {
        if (!ecommerce || !window.dataLayer) return;
        var ctx = context || 'ecommerce';
        console.groupCollapsed('[Ecommerce][' + ctx + '] Pushing to dataLayer');
        ...
        try {
            console.warn('[Ecommerce][' + (context || 'ecommerce') + '] dataLayer push error:', e);
        } catch (_) {
        }
    }
}

function process_nf_link(e, $el) {
    e.preventDefault();
    var $this = $el, action = $this.data('action'), source = $this.attr('data-source'),
        notification_timeout = parseInt($this.data('notification_timeout')) || 5000,
        source_target = $this.data('source_target'), $html_container = $($this.attr('data-html_selector')),
        $to_block = $($this.attr('data-to_block')), icon_selector = $this.data('icon'),
        icon_class = $this.data('icon_success_class'), icon_success_class = $this.data('icon_class') || icon_class,
        icon_wait_selector = $this.data('icon_wait'), remove_path = $this.data('remove_path'), post_data = {};
    $.each($this.data(), function (k, val) {
        if (k.indexOf('postdata') !== -1) {
            post_data['' + k.replace('postdata_', '').replace('postdata', '').toLowerCase()] = val;
    ...
    $.ajax({
        type: 'POST', url: action, data: post_data, success: function (data) {
            try {
                data = $.parseJSON(data);
            } catch (e) {
            }
            try {
                ...
                if (data.favs) {
                    try {
                        Vue.set(vue_simple_store, 'favs', data.favs);
                    } catch (e) {
                        console.debug(e);
                    }
                }
                ...
                make_qty_buttons();
                reach_goal($this);
                pushEcommerceToDataLayer(data.ecommerce, 'add-to-cart');
            } catch (e) {
                console.log(e);
                $($to_block).unblock();
                console.debug(e);
...
var update_checkout = function () {
    var data = $('form.checkout-form').ajaxSubmit({
        url: $('form.checkout-form').attr('data'), 'success': function (data) {
            try {
                data = $.parseJSON(data);
            } catch (e) {
                console.debug(e);
...
function remove_product_widget_from_list(parent_pk, path) {
    if (window.location.pathname === path) {
        if (parent_pk) {
            $('.content').find('.product_widget_block_' + parent_pk).hide('slow', function () {
                $(this).remove();
            });
        }
    }
...
$(function () {
    ...
    $('input.variant').on('change', function () {
        var url = $(this).parents('table.product-variants').attr('data'), variant_id = $(this).attr('value');
        $('#product-form').ajaxSubmit({
            url: url, data: {'variant_id': variant_id}, success: function (data) {
                try {
                    data = $.parseJSON(data);
                } catch (e) {
                    console.debug(e);
                ...
                $.jGrowl(data.message);
                $('a.product-image').lightBox({'txtImage': 'Image', 'txtOf': ' of '});
            }
        });
    });
    $('select.property').on('change', function () {
        $('#product-form').ajaxSubmit({
            url: $('#product-form').attr('data'), success: function (data) {
                try {
                    data = $.parseJSON(data);
                } catch (e) {
                    console.debug(e);
    ...
    $('select.cp-property').on('change', function () {
        $('#product-form').ajaxSubmit({
            url: $('#cp-url').attr('data'), success: function (data) {
                try {
                    data = $.parseJSON(data);
                } catch (e) {
                    console.debug(e);
    ...
    $('.add-accessory-link').on('click', function () {
    ...
    $('body').on('click', '.delete-cart-item', function (e) {
        ...
        $.blockUI({
            css: {border: 'none', background: 'none'},
            overlayCSS: {cursor: 'wait', borderRadius: '4px', padding: '0px'},
            message: '<i class="fa fa-circle-o-notch fa-inverse fa-spin fa-3x">'
        });
        $.post(url, function (data) {
            data = $.parseJSON(data);
            $('#cart-inline').html(data.rendered_cart_inline);
            $.each(data.html, function (index, item) {
                $(item.selector).html(item.html);
            });
            if (data.message) {
                jgrowl_notify({
                    ...
                    button_url: data.button_text,
                    button_class: ''
                });
            }
            pushEcommerceToDataLayer(data.ecommerce, 'delete-item');
            make_qty_buttons();
            $.unblockUI();
        }).error(function (data) {
            make_qty_buttons();
    ...
    $('body').on('change', '.cart-amount', function () {
        ...
        var $to_block = $([$($(this).attr('data-widget_selector'))]);
        $.blockUI({
            css: {border: 'none', background: 'none'},
            overlayCSS: {cursor: 'wait', borderRadius: '4px', padding: '0px'},
            message: '<i class="fa fa-circle-o-notch fa-inverse fa-spin fa-3x">'
        ...
        if ($('#cart-form').length == 0) {
            $('#checkout_virtual_form').wrap('<form id="cart-form"\n' + '                          action="/refresh-cart?ajax"\n' + '                          onsubmit="return false;"></form>');
        }
        $('#cart-form').ajaxSubmit({
            'type': 'post', 'success': function (data) {
                ...
                $('.in_cart_wrap_' + data.product_id).show();
                $('.in_cart_amount_' + data.product_id).html(data.product_amount);
                $('#cart-inline').html(data.rendered_cart_inline);
                if (data.message) {
                    jgrowl_notify({
                        life: 6000,
                ...
                pushEcommerceToDataLayer(data.ecommerce, 'refresh-cart');
                $('#checkout_virtual_form').unwrap();
                make_qty_buttons();
                $.unblockUI();
            }, 'error': function (data) {
                $('#checkout_virtual_form').unwrap();
    ...
    $('.cart-country').on('change', function () {
        $.ajaxSetup({
            beforeSend: function (xhr, settings) {
                if (!csrfSafeMethod(settings.type) && !this.crossDomain) {
                    xhr.setRequestHeader('X-CSRFToken', csrftoken);
    ...
    $('.cart-shipping-method').on('change', function () {
        $('#cart-form').ajaxSubmit({
            'type': 'post', 'success': function (data) {
                try {
                    data = $.parseJSON(data);
    ...
    $('.cart-payment-method').on('change', function () {
        $('#cart-form').ajaxSubmit({
            'type': 'post', 'success': function (data) {
                try {
                    data = $.parseJSON(data);
    ...
    (function () {
        $('#button-cart').on('click', function (e) {
            if ($('#product-form').find('input:radio') && !$('#product-form').find('input:radio:checked').length) {
                var inner = '<div class="success" style="display: block;">Выберите один из вариантов товара<span class="close"><i class="icon-remove-sign"></i></span></div>';
                $('#notification').html(inner).show(3000).hide(1000);
            } else {
                $('#product-form').submit();
            }
        });
    ...
    $('#id_no_shipping').on('click', function () {
        var table = $('.shipping-address');
        if ($('#id_no_shipping:checked').val() !== null) {
            table.slideUp('fast');
        } else {
        ...
        var data = $('.checkout-form').ajaxSubmit({
            url: $('.checkout-form').attr('data'), 'success': function (data) {
                try {
                    data = $.parseJSON(data);
                } catch (e) {
                    console.debug(e);
    ...
    var update_invoice_address = function () {
        var data = $('.postal-address').ajaxSubmit({
            url: $('.postal-address').attr('invoice'), 'success': function (data) {
                try {
                    data = $.parseJSON(data);
                } catch (e) {
                    console.debug(e);
    ...
    var update_shipping_address = function () {
        var data = $('.postal-address').ajaxSubmit({
            url: $('.postal-address').attr('shipping'), 'success': function (data) {
                try {
                    data = $.parseJSON(data);
                } catch (e) {
                    console.debug(e);
    ...
    $('#id_invoice-firstname,#id_invoice-lastname,#id_invoice-line1,#id_invoice-line2,#id_invoice-city,#id_invoice-state,#id_invoice-cpde').on('change', function () {
        save_invoice_address();
    });
    $('body').on('click', '.update-checkout', function () {
        update_checkout();
    });
    $('body').on('change', '.checkout-amount', function () {
        update_checkout();
    });
    $('#id_invoice-country').on('change', function () {
        update_invoice_address();
        update_checkout();
    });
    $('#id_shipping-country').on('change', function () {
        update_shipping_address();
        update_checkout();
    });
    $('#id_shipping-firstname,#id_shipping-lastname,#id_shipping-line1,#id_shipping-line2,#id_shipping-city,#id_shipping-state,#id_shipping-cpde').on('change', function () {
    ...
    $('#empty_voucher').on('click', function (e) {
        e.preventDefault();
        var voucher_input = $('#coupon_ref').first();
        voucher_input.val('');
        voucher_input.attr('value', '');
    ...
    $('.cart-slot').on('mouseenter', function (e) {
        $(this).children('.content').slideToggle(300);
    });
    $('.cart-slot').on('mouseleave', function (e) {
        $(this).children('.content').slideToggle(300);
    });
});

function get_template(tpl_name) {
    return $('#' + tpl_name).html();
}
...
function reach_goal(el) {
    var goal_name, visit_params = {};
    $.each(el.data(), function (k, val) {
        if (k.indexOf('vp__') !== -1) {
            if (k.replace('vp__', '') === 'goal_name') {
                goal_name = val;
            } else {
                visit_params[k.replace('vp__', '').toLowerCase()] = val;
            }
        }
    });
    try {
        if (!~window.location.href.indexOf('localhost')) {
            ym(30941981, 'reachGoal', goal_name, visit_params);
        } else {
...
$(document).ready(function () {
    ...
    $('.go_search').on('click', function () {
        var search_input = $('#q'), this_cat_checkbox = $('#this_category');
        var url = '/search?q=' + search_input.val();
        if (this_cat_checkbox.prop('checked')) {
            url += '&this_category=1';
            url += '&category_id=' + $('#q').attr('data-category_id');
        }
    ...
    $body.on('click', '.plus', function (e) {
        ...
        var _value = parseInt(inp.val()) + 1;
        inp.val(_value);
        inp.attr('value', _value);
        inp.change();
    });
    ...
    $('#callback, .close_login, .overlay').on('click', function (e) {
        e.preventDefault();
        toggle_popup();
    });
    $('.summ_input').on('change', function (e) {
        $('.total_sum').html(($('.hidden_price').first().val() * $(this).val()).toFixed(2));
        $($(this).attr('data-target_selector')).find('[name=quantity]').attr('value', $(this).val());
    });
    $body.on('click', '.js_ajax_post_nf_link', function (e) {
        process_nf_link(e, $(this));
    });
    $('.clear_phone').on('click', function (e) {
        e.preventDefault();
        $('#inactive_phone1').html('');
        $('#id_invoice-phone1').val('');
        $('.clear_phone').hide();
    });
    $('body').on('click', '#submit_order', function (e) {
        e.preventDefault();
        $.blockUI({
            css: {borderRadius: '6px', border: 'none', background: 'none'},
            overlayCSS: {cursor: 'wait'},
            message: '<i class="fa fa-circle-o-notch fa-5x fa-inverse fa-spin"></i>' + '<div style="text-align: center; margin-top: 30px; color: #fff; font-size: 24px">Отправляем заказ...</div>'
        });
        if ($('#id_invoice-phone1').val() == '+7' || $('#id_invoice-phone1').val() == '+7 ') {
            $('#id_invoice-phone1').val('');
        }
        if ($('#id_invoice-phone').val() == '+7' || $('#id_invoice-phone').val() == '+7 ') {
            $('#id_invoice-phone').val('');
        }
        $('#confirmation-form').submit();
    });
    $('body').on('click', '.checkout-radio-button ', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var $to_block = $('.checkout_shipping_payment');
        $to_block.block({
            css: {border: 'none', background: 'none'},
            overlayCSS: {cursor: 'wait'},
            message: '<i class="fa fa-circle-o-notch fa-2x fa-inverse fa-spin"></i>' + '<div style="text-align: center; color: #fff; font-size: 14px"></div>'
        });
        $(this).find('input').prop('checked', 'checked').change();
        update_checkout();
    });
    $('body').on('click', '.choose_date', function (e) {
        e.preventDefault();
        e.stopPropagation();
        $('.datetime').focus();
    });
    $('.call_back_button').on('click', function () {
        if ($('#phone_message').val().length == 16) {
            $to_block = $('.call_back_modal').find('.modal-content');
            $to_block.block({
                css: {border: 'none', background: 'none'},
                overlayCSS: {cursor: 'wait'},
                message: '<i class="fa fa-circle-o-notch fa-2x fa-inverse fa-spin"></i>' + '<div style="text-align: center; color: #fff; font-size: 14px">Сообщение отправляется...</div>'
            });
            $('#contactForm').ajaxSubmit({
                'type': 'post', 'success': function (data) {
                    ...
                    $('#phone_message').val('');
                    $to_block.unblock();
                    $('#contactForm').siblings('.success_message').toggle();
                    ...
                    $('#phone_message').val('');
                    $to_block.unblock();
                    $('#contactForm').siblings('.error_message').show();
                    window.setTimeout(function () {
                        $('.close').click();
                        $('#contactForm').toggle();
                        $('#contactForm').siblings('.error_message').toggle();
                    }, 5000);
                }
            });
        }
    ...
    $('body').on('click', '.box-collateral', function () {
        $(this).find('.box-collateral-content').slideToggle();
    });
    $('body').on('click', '.sf-menu-block', function () {
...
function initpostsize() {
    if ($(".post-block").length > 0) {
        if ($(window).width() < 560 && $(".line-post").length > 0) {
            $(".post-block").removeClass("line-post");
        }
        $('body').waitForImages(function () {
            $(".post-title").removeAttr("style");
            var ps = 0;
...
function catheight() {
    var vr = 0;
    $(".cat-height").removeAttr("style");
    $(".cat-height").each(function () {
        if ($(this).height() > vr) {
            vr = $(this).height();
        }
        $(".cat-height").height(vr);
    });
}

$(document).ready(function () {
    if ($(".cat-height").length > 0) {
        catheight();
    }
...
$(document).ready(function () {
    initpostsize();
    $(".mainwrapper").prepend("<div class='mobile-header wfb'></div>");
    $(".header .logo").clone().appendTo(".mobile-header");
    $(".mobile-header").append("<div class='buttons'> <a href='/cart'><i class='fa fa-shopping-cart'></i></a> <a href='' class='open-search'><i class='fa fa-search'></i></a> <a href='' class='open-menu'><i class='fa fa-navicon'></i></a></div>");
    $(".header .search").clone().appendTo(".mobile-header").wrap("<div class='mobile-search'></div>");
    $(".mainwrapper").prepend("<div class='float-menu'><div class='float-menu__wrap'><div class='mobile-nav'><b class='float-menu__title'>Навигация:</b></div><div class='user-nav'><b class='float-menu__title'>Профиль:</b></div></div></div>");
    $(".main-nav").clone().appendTo(".mobile-nav");
    $(".top-links").clone().appendTo(".user-nav");
    $('.js-read-search').on('keyup', function (e) {
        $('#hq').val($(this).val());
    });
    var current_search = null;
    $('.js-read-search').on('keyup', function (e) {
        $('.default_search_title').hide();
        $('.search_spinner').show();
        if (current_search) {
            current_search.abort();
        }
        var self = $(e.target), cursor_position = self.getCursorPosition();
        if (e.keyCode === 27) {
        ...
        if ([9, 16, 17, 18, 19, 20, 27, 33, 34, 35, 36, 37, 38, 39, 40, 91, 92, 93].indexOf(e.keyCode) !== -1) {
            return false;
        }
        $(".js-read-search").val(self.val());
        self.setCaret(cursor_position);
        var search_modal = $(".search-modal"), search_input = $('.js-read-search');
        if (self.val().length > 0) {
            var search_url = search_input.data('url') + '?' + search_input.attr('name') + '=' + search_input.val();
            current_search = $.get(search_url, function (data) {
                search_modal.find('.search-modal__wrap').children().remove();
                if (data.state === 'success') {
                    search_modal.find('.search-modal__wrap').append(data.products);
                    search_modal.addClass("active");
                    search_modal.addClass("open");
                    $('.default_search_title').show();
                    $('.search_spinner').hide();
                }
            });
    ...
    $(".js-read-search").click(function (e) {
        if ($(this).val().length && !$(".search-modal").hasClass('active')) {
            $(".search-modal").addClass('active');
        }
    });
    $(".js-read-search").on('focus', function () {
        if ($(this).val() && $(this).siblings('.search-modal').find('.search-modal__wrap').children().length) {
            $(this).siblings('.search-modal').addClass('active');
        }
    });
    if ($(".main-block .bx-slider").length > 0) {
        $(".bx-slider").bxSlider({controls: false, pager: true, mode: 'fade', auto: true, pause: 6000,});
    }
    $("body").on("click", ".hasmenu .nav-link", function (e) {
        $(this).parent(".hasmenu").toggleClass("active");
        e.preventDefault();
    });
    $("body").on("click", ".open-menu", function (e) {
        $(".float-menu").toggleClass("menu-active");
        $(".open-menu").toggleClass("active");
        e.preventDefault();
    });
    $(".product-small-img").click(function (e) {
        $(".product-small-img").removeClass("active");
        $(this).addClass("active");
        $(".product-main-img img").attr("src", $(this).attr('href'));
        $(".product-main-img").attr("href", $(this).attr('href'));
        $(".product-main-img").attr("data-title", $(this).children('img').attr('title'));
        e.preventDefault();
    });
    $("body").on("click", ".open-search", function (e) {
        if ($(".open-search").hasClass("active")) {
            $(".open-search").removeClass("active");
            $(".mobile-search").animate({height: "0px"}, 300, function () {
                $(this).removeClass("active");
            });
        } else {
            $(".open-search").addClass("active");
            $(".mobile-search").animate({height: "63px"}, 300, function () {
                $(this).addClass("active");
            });
        }
        e.preventDefault();
    });
    $(".js-int-sale").on('click', function (e) {
        $(".js-hidden-box").toggleClass("active");
    });
    $(".close-link").click(function (e) {
        $(".js-hidden-box").toggleClass("active");
        $(".js-int-sale").removeProp("checked");
    });
    $("body").on("click", ".filter-block a", function (e) {
        if (!$(this).hasClass("disabled")) {
            $(this).toggleClass("active");
        }
    });
    $(".clean-filter").click(function (e) {
        $(".filter-block a").removeClass("active");
    });
    $(".js-type-1").click(function (e) {
        $(".posts-shop .post-block").removeClass("line-post");
        $(".view-type a").removeClass("active");
        $(this).addClass("active");
        initpostsize();
        e.preventDefault();
    });
    $(".js-type-2").click(function (e) {
        $(".posts-shop .post-block").addClass("line-post");
        $(".view-type a").removeClass("active");
        $(this).addClass("active");
        initpostsize();
        e.preventDefault();
    ...
    $(document).click(function (event) {
        ...
        if (!$(event.target).closest(".float-menu, .open-menu").length) {
            $(".float-menu").removeClass("menu-active");
            $(".open-menu").removeClass("active");
        }
        if (!$(event.target).closest(".mobile-search, .open-search").length) {
            $(".open-search").removeClass("active");
            $(".mobile-search").animate({height: "0px"}, 300, function () {
                $(this).removeClass("active");
            });
    ...
    $('body').on('click', '.js_ajax_post_link', function (e) {
        e.preventDefault();
        var source = $(this).attr('data-source'), source_target = $(this).attr('data-source_target'),
            html_container = $($(this).attr('data-html_selector')), $to_block = $($(this).attr('data-html_selector')),
            $goal = $(this).attr('data-goal-name'), icon = $(this).find('.fa'), $this = $(this);
        $.ajaxSetup({
            beforeSend: function (xhr, settings) {
                if (!csrfSafeMethod(settings.type) && !this.crossDomain) {
                    xhr.setRequestHeader("X-CSRFToken", csrftoken);
        ...
        $($to_block).each(function () {
            $(this).block({
                css: {border: 'none', background: 'none'},
                overlayCSS: {cursor: 'wait', padding: '0px'},
                message: '<i class="fa fa-circle-o-notch fa-inverse fa-spin fa-3x"></i>'
            })
        });
        $($(this).attr('data-target_selector')).ajaxSubmit({
            "type": "post", "success": function (data) {
                try {
                    data = $.parseJSON(data);
                    ...
                    if (!_.isUndefined($goal)) {
                        ym(30941981, 'reachGoal', $goal);
                    }
                    $.jGrowl(tpl, {life: 5000, theme: 'alert alert-success'});
                } catch (e) {
                    $($to_block).unblock();
                    console.debug(e);
                }
                ...
                console.log(e);
                icon.toggleClass('fa-heart-o').toggleClass('fa-spinner').toggleClass('fa-spin');
                $($to_block).unblock();
                var tpl = get_template('common_error');
                tpl = _.template(tpl)();
                $.jGrowl(tpl, {life: 5000, theme: 'alert alert-error'});
            }
        });
    });
    $('body').on('click', '.cat_link', function (e) {
        e.preventDefault();
    ...
    $('body').on('click', '.shipping_payment_additional_param_checkbox', function (e) {
        e.stopPropagation();
    });
    $('body').on('click', '.shipping_payment_additional_param_checkbox_label', function (e) {
        e.stopPropagation();
    });
    $('body').on('change', '.subscrition_checkbox', function () {
    });
});
;/*!
 * Lightbox v2.8.1
...
function get_user_layer_data() {
    var xhr = new XMLHttpRequest();
    xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 300) {
            var data = JSON.parse(xhr.responseText);
            if (data.cart) {
                $('.mini-cart-content').html(data.cart)
            }
            if (data.favs) {
                try {
                    vue_simple_store.setFavsAction(data.favs)
                ...
                for (var id in data.favs) {
                    var selector = '.add_remove_favorite[data-postdata-product_id=' + data.favs[id] + '] i';
                    $(selector).addClass('fa-heart').removeClass('fa-heart-o');
                }
            }
            if (data.in_cart_items) {
                var n = 0;
                for (var i = 0; i < data.in_cart_items.length; i++) {
                    n += Number(data.in_cart_items[i].amount) || 0
                }
                $('.cart-items-amount').html(n);
                if (n) {
                    $('.cart-items-amount').show(400)
                }
                try {
                    vue_simple_store.setCartItemsAction(data.in_cart_items)
                } catch (e) {
                    console.debug(e)
                }
                for (var i in data.in_cart_items) {
                    var span_selector = '.in_cart_amount_' + data.in_cart_items[i].id;
                    $(span_selector).addClass('has-badge').attr('data-count', data.in_cart_items[i].amount);
                }
            }
        } else {
        }
    };
    xhr.open('GET', '/user_layer_data');
    xhr.send();
}
...
(function (window, undefined) {
    History.Adapter.bind(window, 'statechange', function () {
        ...
        if (State.url.indexOf(window.location.pathname) !== -1 && (State.url.indexOf('start=') !== -1 || State.url.indexOf('f=') !== -1 || State.url.indexOf('preorder=') !== -1 || State.url.indexOf('in_stock=') !== -1) || State.data.preorder_or_in_stock == 1) {
            $('.center-block').block({
                css: {border: 'none', background: 'none'},
                overlayCSS: {
                    cursor: 'wait',
                    ...
                    outline: '0px solid lavenderblush',
                    padding: '0px',
                    backgroundColor: 'lavenderblush'
                },
                message: '<i class="fa fa-spinner fa-3x fa-inverse fa-spin" style="color: lightslategray; position: fixed; top: 190px;"></i>'
            });
            $.post(State.url, {ajax: 1}, function (data) {
                try {
                    var json_data = JSON.parse(data);
                    $('.category_inline').html(json_data.html);
                    $('.center-block').unblock();
                } catch (e) {
                    console.log(e);
                    $('.center-block').unblock();
                }
...
function jgrowl_notify(options) {
    var life = options.life || 6000, level = options.level || 'info', header = options.header || '',
        message = options.message || '', image = options.image || '', button_text = options.button_text || '',
        button_url = options.button_url || '', button_class = options.button_class || 'btn btn-small btn-circle',
        theme = 'alert alert-' + level
    var jgrowl_notify_template = get_template('jgrowl_notify_template')
    var tpl = _.template(jgrowl_notify_template)({
        level: level,
        header: header,
        message: message,
        image: image,
    ...
    $.jGrowl(tpl, {life: life, theme: theme})
};$(function () {
    $('img').Lazy();
    $('.info_popup').popup({scrolllock: true, transition: 'all 0.3s', vertical: 'center'});
    var headerCart = $('.header-cart');
    var miniCartWrap = $('.mini-cart-wrap');
    var miniCartContent = $('.mini-cart-content');
    var cartOverlay = $('.cart-overlay');
    var savedCartContent = null;

    function openDrawer(mode) {
        miniCartWrap.toggleClass('is-filters', mode === 'filters');
    ...
    function closeDrawer(e) {
        ...
        if (miniCartWrap.hasClass('is-filters') && savedCartContent !== null) {
            miniCartContent.html(savedCartContent);
            savedCartContent = null;
        }
        cartOverlay.removeClass('visible');
        miniCartWrap.removeClass('open is-filters');
        $('body').css('overflow', '');
    }

    function buildFiltersHtml(source) {
        var panel = source.find('.filters-panel').first();
        if (panel.length) {
            return $('<div class="mobile-filters-only"></div>').append(panel.clone()).html();
        }
        var wrap = $('<div class="mobile-filters-only"></div>');
        var filterHeader = source.find('.clean-filter').first().closest('.shop-sidebar, .filters-panel-head');
        var filterBlocks = source.find('.filter-block, .filters-group');
        var resetButton = source.find('.filters-reset, .mb-45.text-center').first();
        if (filterHeader.length) {
            wrap.append(filterHeader.clone());
        }
        ...
        if (resetButton.length) {
            wrap.append(resetButton.clone());
        }
        return $.trim(wrap.html());
    }

    headerCart.on('click', function (e) {
        e.preventDefault();
        if (savedCartContent !== null) {
            miniCartContent.html(savedCartContent);
            savedCartContent = null;
        }
        openDrawer('cart');
    ...
    $(document).on('click', '[data-open-filters-drawer]', function (e) {
        ...
        filtersHtml = buildFiltersHtml(source);
        if (!filtersHtml) {
            return;
        }
        e.preventDefault();
        savedCartContent = miniCartContent.html();
        miniCartContent.html(filtersHtml);
        openDrawer('filters');
    });
    $(document).on('click', '.close-cart, .cart-overlay', closeDrawer);
});
...
function styleDifferences(a, b) {
    ...
    var formWrapper = document.querySelector('.review-form-wrapper');
    if (formWrapper) {
        var productSlug = formWrapper.getAttribute('data-product-slug');
        if (productSlug) {
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '/shop_reviews/product/' + productSlug + '/form-fragment', true);
            xhr.onload = function () {
                if (xhr.status === 200) {
